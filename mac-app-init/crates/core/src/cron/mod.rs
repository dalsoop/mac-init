...
pub fn cron_matches(expr: &str, min: u32, hour: u32, day: u32, month: u32, weekday: u32) -> bool {
    ...
    fn fm(field: &str, value: u32) -> bool {
        ...
        if field.contains(',') {
            return field
                .split(',')
                .any(|f| f.parse::<u32>().ok() == Some(value));
        }
        false
    }
    fm(parts[0], min)
...
pub fn tick() {
    ...
    let mut last_run: HashMap<String, u64> = if last_run_path.exists() {
        serde_json::from_str(&fs::read_to_string(&last_run_path).unwrap_or_default())
            .unwrap_or_default()
    } else {
        HashMap::new()
    };
    ...
    fs::write(
        &last_run_path,
        serde_json::to_string(&last_run).unwrap_or_default(),
    )
    .ok();
}
...
fn is_mine(label: &str) -> bool {
    ...
    label.starts_with("com.mac-host") || label.starts_with("com.mac-init") || label.contains(&user)
}
