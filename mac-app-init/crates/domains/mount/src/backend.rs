...
impl MountOpts {
    ...
    pub fn smbfs_opts_string(&self) -> String {
        ...
        if self.nobrowse {
            v.push("nobrowse".into());
        }
        if self.rsize > 0 {
            v.push(format!("rsize={}", self.rsize));
        }
        if self.wsize > 0 {
            v.push(format!("wsize={}", self.wsize));
        }
...
pub fn mount(
    req: &MountRequest<'_>,
    smbfs_fallback: impl Fn(&MountRequest<'_>) -> Result<(), String>,
...
pub fn rclone_mount(
    ...
    let child = cmd
        .spawn()
        .map_err(|e| format!("rclone 실행 실패: {} (https://rclone.org/downloads/)", e))?;
