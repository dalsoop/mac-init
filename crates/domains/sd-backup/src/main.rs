...
fn config_path() -> PathBuf {
    PathBuf::from(home()).join(".mac-app-init/sd-backup.json")
}
fn history_path() -> PathBuf {
    PathBuf::from(home()).join(".mac-app-init/sd-backup-history.json")
}
...
fn progress_path() -> PathBuf {
    PathBuf::from(home()).join(".mac-app-init/sd-backup-progress.json")
}
...
fn lock_path() -> PathBuf {
    PathBuf::from(home()).join(".mac-app-init/sd-backup.lock")
}
...
fn detect_device_type(vol: &Path) -> String {
    ...
    // 볼륨 이름으로 빠른 판별
    if vol_name.contains("EOS") {
        return detect_canon(&dcim, &vol_name);
    }
    ...
    if let Ok(it) = fs::read_dir(&dcim) {
        for e in it.filter_map(|x| x.ok()) {
            ...
            if n.contains("CANON") || n.starts_with("CANONMSC") {
                return detect_canon(&dcim, &vol_name);
            }
            if n.contains("NIKON") || n.starts_with("NIKON") {
                return "Nikon".into();
            }
            if n.contains("GOPR") || n.contains("HERO") {
                return "GoPro".into();
            }
            if n.starts_with("FUJI") {
                return "Fujifilm".into();
            }
        }
    }

    // Sony: PRIVATE/M4ROOT/
    if vol.join("PRIVATE/M4ROOT").exists() {
        return "Sony".into();
    }
...
fn detect_canon(dcim: &Path, vol_name: &str) -> String {
    if let Ok(it) = fs::read_dir(dcim) {
        for e in it.filter_map(|x| x.ok()) {
            ...
            if n.contains("EOSR6") {
                return "Canon-EOS-R6".into();
            }
            if n.contains("EOSR5") {
                return "Canon-EOS-R5".into();
            }
            if n.contains("EOSR3") {
                return "Canon-EOS-R3".into();
            }
            if n.contains("EOSR7") {
                return "Canon-EOS-R7".into();
            }
            if n.contains("EOSR") {
                return "Canon-EOS-R".into();
            }
            if n.contains("EOS5D") {
                return "Canon-5D".into();
            }
        }
    }
    if vol_name.contains("EOS") {
        return "Canon-EOS".into();
    }
    "Canon".into()
}
...
fn count_media(dcim: &Path) -> (usize, u64) {
    ...
    if let Ok(output) = Command::new("find")
        .args([dcim.to_str().unwrap_or("."), "-type", "f"])
        .output()
    {
    ...
    match status {
        Ok(s) if s.success() => {
            ...
            if staged_size != file_size {
                return CopyResult::SizeMismatch {
                    expected: file_size,
                    actual: staged_size,
                };
            }
...
fn cmd_status() {
    ...
    if cards.is_empty() {
        ...
        for card in &cards {
            ...
            println!(
                "  전체: {}개 ({})",
                card.file_count,
                human_bytes(card.total_bytes)
            );

            // 차분 분석: 이미 백업된 파일 vs 새 파일
            if !cfg.backup_target.is_empty() {
                if let Some(dcim) = &card.dcim_path {
                    let (new_count, new_bytes, existing) =
                        diff_analysis(dcim, &cfg, &card.device_type);
                    if new_count == 0 {
                        println!("  상태: ✓ 이미 백업 완료 (새 파일 없음)");
                    } else if existing > 0 {
                        println!(
                            "  상태: ⚡ 추가 파일 {}개 ({}) — 기존 {}개 skip",
                            new_count,
    ...
    if let Some(last) = hist.entries.last() {
        println!(
            "\n최근 백업: {} | {} | {}개 파일",
            last.timestamp, last.device, last.files_copied
        );
    }
}
...
fn diff_analysis(dcim: &Path, cfg: &Config, device_type: &str) -> (usize, u64, usize) {
    ...
    for f in &files {
        let fname = f
            ...
            .to_string_lossy()
            .to_string();
        if !cfg.include_lrf && fname.to_uppercase().ends_with(".LRF") {
            continue;
        }
...
fn cmd_run() {
    ...
    for card in &cards {
        ...
        let Some(dcim) = &card.dcim_path else {
            continue;
        };
        ...
        let bytes_total: u64 = eligible_files
            .iter()
            ...
            for src in files {
                ...
                match copy_staged(src, &dest, &staging) {
                    ...
                    CopyResult::SizeMismatch { expected, actual } => {
                        eprintln!(
                            "  ✗ 크기 불일치: {} (src={}B staged={}B) — .partial/에 잔류",
    ...
    let sync_msg = if cfg.sync_enabled {
        " → NAS 동기화"
    } else {
        ""
    };
    ...
    let _ = Command::new("osascript")
        .args([
            "-e",
            &format!(
                "display notification \"{}\" with title \"mac-app-init\"",
                msg
            ),
        ])
        .output();
...
fn cmd_eject_toggle(toggle: &str) {
    ...
    match toggle.to_lowercase().as_str() {
        ...
        "status" => {
            println!(
                "자동 추출: {}",
                if cfg.auto_eject { "켜짐" } else { "꺼짐" }
            );
        }
        _ => {
            eprintln!("사용법: sd-backup eject <on|off|status>");
            std::process::exit(1);
        }
    }
    save_config(&cfg);
    ...
    match status {
        Ok(s) if s.success() => {
            ...
            let _ = Command::new("osascript")
                .args([
                    "-e",
                    "display notification \"SD 카드 안전 추출됨\" with title \"mac-app-init\"",
                ])
                .output();
        }
        _ => eprintln!("  ✗ SD 추출 실패"),
...
fn cmd_auto(toggle: &str) {
    ...
    match toggle.to_lowercase().as_str() {
        ...
        "status" => {
            println!(
                "자동 백업: {}",
                if cfg.auto_enabled { "켜짐" } else { "꺼짐" }
            );
            println!(
                "LaunchAgent: {}",
...
fn cmd_progress() {
    ...
    println!(
        "용량  : {} / {} ({}%)",
        human_bytes(prog.bytes_done),
        human_bytes(prog.bytes_total),
        pct
    );
...
fn cmd_history() {
    ...
    for (i, e) in hist.entries.iter().rev().enumerate() {
        ...
        println!(
            "  복사    : {}개 (skip {}개)",
            e.files_copied, e.files_skipped
        );
...
fn print_tui_spec() {
    ...
    let target_status = if cfg.backup_target.is_empty() {
        "미설정"
    } else if Path::new(&expand(&cfg.backup_target)).exists() {
        "✓ 접근 가능"
    } else {
        "✗ 경로 없음"
    };

    let last_backup = hist
        .entries
        .last()
    ...
    let prog_info = if prog.running {
        ...
        format!(
            "▶ 백업 중 ({}/{} 파일, {}%) — {}",
            prog.files_done, prog.files_total, pct, prog.current_file
        )
    } else {
        "대기 중".into()
    };
    ...
    let usage_summary = if cfg.auto_enabled {
        "자동백업 켜짐".to_string()
    } else {
        "꺼짐".to_string()
    };
