...
fn print_tui_spec() {
    ...
    let usage_summary = if auto_on { "자동정리 켜짐".to_string() } else { "꺼짐".to_string() };

    TuiSpec::new("files")
        .refresh(30)
        .usage(usage_active, &usage_summary)
        .kv("상태", vec![
            tui_spec::kv_item("Downloads", &format!("{}개 파일", dl_count),
                if dl_count > 20 { "warn" } else { "ok" }),
            tui_spec::kv_item("임시 폴더", &format!("{}개 파일", tmp_count),
                if tmp_count > 10 { "warn" } else { "ok" }),
            tui_spec::kv_item("자동 정리",
